		...
		if (path === '/api/virtualbox') {
			if (method === 'GET') {
				...
				if (params.has('id')) {
					...
					const deleteStorageRequest = new Request(`${env.STORAGE_INITIAL_URL}/api/project`, {
						method: 'DELETE',
				...
				const initStorageRequest = new Request(`${env.STORAGE_INITIAL_URL}/api/init`, {
					method: 'POST',
